...
save_t	*check_tetrimino(char **tab, int w, int c, save_t *list)
{
	int min = -1;

	if (get_min_and_max(tab, w, &min) == -1)
		return (list);
	for (int i = 0; tab[i]; i += 1)
		tab[i] = change_line(tab[i], w, min);
	list = add_tetrimino(tab, list, c);
	return (list);
}

save_t	*next_tetrimino(char *path, save_t *list)
{
	...
	tab[h] = NULL;
	close(fd);
	return (check_tetrimino(tab, w, c, list));
}
...
save_t	*get_tetrimino(int *len)
{
	...
	dir = readdir(fd);
	while (dir) {
		list = next_tetrimino(my_strcat("tetriminos/",
		dir->d_name), list);
		dir = readdir(fd);
	}
	closedir(fd);
